  ...
  class FreeSortPlugin {
      ...
      trial(display_element, trial) {
          ...
          if (!trial.stim_starts_inside) {
              ...
              //     for (const y of make_arr(0, trial.sort_area_height - trial.stim_height, num_rows, true)) {
              ...
              // console.log(r_coords);
              // console.log(l_coords);

              // Hardcode the coordinates for 6 items.

              // var r_coords = [{x: -75, y: -25}, {x: 50, y: -25}, {x: 150, y: -25}, {}, {}, {}];

              // var l_coords = [{x: -25, y: -25}, {x: 100, y: -25}, {x: 200, y: -25}, {}, {}, {}];

              var t_coords = [{x: -150, y: -100}, {x: -50, y: -100}, {x: 50, y: -100}, {x: 150, y: -100}, {x: 250, y: -100}, {x: 350, y: -100}];
          ...
          for (let i = 0; i < stimuli.length; i++) {
              var coords;
              if (trial.stim_starts_inside) {
                  coords = random_coordinate(i, trial.sort_area_width - trial.stim_width, trial.sort_area_height - trial.stim_height);
          ...
          let t_count = inside.reduce(function(a,b){return a+b});
          // console.log(t_count);

          if (t_count !== 2 && trial.change_border_background_color) {
              // console.log('errrm')
          ...
          for (let i = 0; i < draggables.length; i++) {
              draggables[i].addEventListener(start_event_name, (event) => {
                  ...
                  var end_event = (e) => {
                      ...
                      //     y: elem.offsetTop,
                      // });
                      // console.log(elem.dataset.src, elem.offsetLeft, elem.offsetTop)
          ...
          function make_arr(startValue, stopValue, cardinality, y = false) {
          ...
          function random_coordinate(i, max_width, max_height) {
              const rnd_x = Math.floor(Math.random() * (max_width - 1));
              const rnd_y = Math.floor(Math.random() * (max_height - 1));
              // console.log(rnd_x, rnd_y);
              return {
                  x: rnd_x,
                  y: rnd_y,
              };
          }
          // function fixed_coordinate(i) {
          //   const f_coords = {
          //     0: {x: y: };
          //   };
          // }
